    ...
    if os.path.exists(json_filepath):
        ...
        try:
            ...
            if "peak_wave_period" in start_content:
                logger.info(f"JSON already exists and contains new vars: {json_filename}")
                return "exists"
            else:
                logger.info(f"JSON exists but missing new vars. Redownloading: {json_filename}")
        except Exception as e:
            logger.warning(f"Error reading existing file {json_filename}: {e}. Overwriting.")
